...
try {
 ...
 const titulo = req.query.titulo;  
  if (apikey === undefined || autor === undefined || nomealbum === undefined || titulo === undefined || fotomusic === undefined || fundo === undefined) return res.sendFile(paramtroerro)
  let check = await verificaKey(apikey)
  if (!check) return res.sendFile(semapikey)
  let limit = await isLimit(apikey);
  if (limit) return res.sendFile(semlimit)
...
var image = await new canvafy.Spotify()
    ...
    .build();
  data = image.toBuffer();
  await fs.writeFileSync(__path +'/tmp/music.png', data)
