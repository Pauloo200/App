...
const { verificaNome, verificaNome2, verificaAdmin, checkVerify, verificaDinheiro2, dinheiroadd, dinheiroretirar, niveladd, nivelretirar, verificanivel, verificanivel2, verificaVerif, expadd, expretirar, verificaexp, verificaexp2, uplvl, addUtil, addRequest,  getTotalReq, getTodayReq, getidveri, verificaAll, verificaZap, tempo_ban, banir, verificar_dias_ban, resetAllLimit, resetarAllTributers, salvardd, verificaNomeig } = require('./backend/db');
...
const { enviarnozap, enviarnozap_button, enviarimg } = require('./japabot.js');
//const { simih } = require('./instabot.js');
//const { setbio } = require('./setbio.js');
...
//cria um middleware pra capturar o tempo de resposta
...
let {
	...
	let imgpadrao = 'https://telegra.ph/file/60b4caf561c9585eb8dd0.jpg'
        let mscpadrao = 'https://japa-api.cf/audio/musica.mp3'
	let cekeban = ms(banido - Date.now())
let expiredban = '0 d'
	if (cekeban !== null) {
		expiredban = cekeban
...
try {
api_tel = await fetchJson(`https://japa-api.cf/tel1/${tel}`)
console.log(api_tel)
...
try {
api_tel = await fetchJson (`https://japa-api.cf/cpf3/${cpf}`)
...
let { nome } = req.body;
try {
api_tel = await fetchJson (`https://japa-api.cf/nome/${nome}`)
console.log(api_tel.resultado)
req.flash('error_msg', `${api_tel.resultado}`);
return res.redirect('/painel');
} catch(err) {
...
let templateButtons = [
  ...
  {index: 3, urlButton: {displayText: '⭐ Falar com Suporte!', url: 'https://wa.me/5516993025827?text=preciso+de+ajuda'}},
]
...
//client.login('_Paulo.js', '34615194');
